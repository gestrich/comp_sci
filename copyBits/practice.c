...
int main(){
  ...
  int x = 5; // 110
  int y = 1; // 0001
  short int sourceIndex = 2;
  short int destinationIndex = 10;
  short int length  = 2;
...
void copyBits(int x, int y, short int sourceIndex, short int length, short int destinationIndex){
  ...
  printf("x: %u y: %u sourceIndex: %u length: %u destinationIndex: %u\n", x, y, sourceIndex, length, destinationIndex);
  ...
  for(int i = destinationIndex; i > (destinationIndex-length) && i >= 0; i--){
    int modRes = y % (int)pow(2, i+1);
    if(modRes >= (int)pow(2, i)){
      y -= (int)pow(2, i);
    }
  }
